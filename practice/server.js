/* 
Copied from Assignment 1 example video, info_server_Ex4.js from Lab13, and lab 14 
Alyssa Mencel's Assignment 2: Server
*/

var data = require('./public/products.js'); //load products.js file and set to variable 'data'
var allProducts = data.allProducts; //set variable 'services_array' to the services_array in the services_data.js file
...
app.post("/generateInvoice", function (request, response) {
    ...
    for (product in allProdu) {
        for (i = 0; i < allProducts[product].length; i++) {

            qty = cart[`${product}${i}`];
            if (qty > 0) { //if there is a quantity entered in the textbox ...
                extended_price = qty * allProduct[product][i].price //equation for extended price
    ...
    var mailOptions = {
        from: 'alyssamencel@gmail.com', //sender is SpanishDayTours@gmail.com
...
app.post("/process_form", function (request, response) { //process the quantity_form when the POST request is initiated to form a response from the values in the form
    ...
    if (typeof POST['addProducttoCart${i}'] != 'undefined') { //if the POST request is not undefined...
        ...
        for (i = 0; i < `${(products_array[`community`][i]) + _products}`.length; i++) { //for any given tour in any community array...
